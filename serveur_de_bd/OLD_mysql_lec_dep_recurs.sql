...
-- call ap_debog ('6');
						set @sql=concat('insert into temp_tab_id(niveau,id) select ',p_niveau,',',v_nom_col_pk_fk,' from ',v_nom_tab2,' where ',v_nom_col_fk,'=',convert(p_id,char));
...
-- call ap_debog ('7');
						...
						set @sql=concat('insert into temp_tab_id(niveau,id) select ',p_niveau,',',v_nom_col_fk,' from ',p_nom_tab,' where ',v_nom_col_fk_pk,'=',convert(p_id,char));
...
-- call ap_debog(concat('v_niv2=',v_niv2));
									if (v_niv2<32 and v_niv2<=p_profondeur) then -- Nombre d'imbrication dans T-sql doit �tre inf�rieure � 32
										if(v_debog=1) then
